...
export const Editar = () => {
  ...
  // Função para buscar o professor a ser editado
  useEffect(() => {
    const fetchProfessor = async () => {
      try {
        const response = await axios.get(`http://localhost:3333/professores/${id}`);
        ...
        setFeedback("Erro ao carregar os dados do professor.");
      }
    };
  ...
  // Função para submeter a edição do professor
  const handleSubmit = async (event) => {
    event.preventDefault();
    const professorEditado = { nome, curso, titulacao, ai, universidade };

    try {
      await axios.put(`http://localhost:3333/professores/${id}`, professorEditado);
      setFeedback("Professor atualizado com sucesso!"); // Mensagem de sucesso
      setTimeout(() => navigate("/professor/listar"), 2000); // Redireciona para a lista após 2 segundos
    } catch (error) {
      setFeedback("Erro ao atualizar o professor. Tente novamente."); // Mensagem de erro
    }
  };
